...
class NMRF(nn.Module):
    ...
    def __init__(self,
        ...
        divis_by = cfgs.DIVIS_BY
        compat = cfgs.COMPAT

        backbone = create_backbone(model_type=model_type, norm_fn='instance', out_channels=out_channels, drop_path=0.4)
        dpn = DPN(cost_group=4, num_proposals=4, feat_dim=out_channels, context_dim=64, num_prop_layers=5, prop_embed_dim=128, mlp_ratio=4, split_size=1, prop_n_heads=4, normalize_before=True)
        weight_dict = {'proposal_disp': 1,
                       ...
                       'loss_disp_8': 2.0,
                       'loss_disp': 2.0}
        criterion = Criterion(weight_dict=weight_dict, max_disp=192, loss_type='L1')
